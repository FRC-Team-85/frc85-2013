 ...
 */
public class HallEffect extends SensorBase implements CounterBase, PIDSource {
    
    public double m_Start_Time = 0;
    public double m_Diff_Time = 0;
    public double m_Prev_Time = 0;
    public double m_Curr_Time = 0;
    
    public double m_Prev_Count = 0;
    public double m_Curr_Count = 0;
    public double m_Diff_Count = 0;
    
    private double m_Return_RPM_Loop_Count = 0;
    
    double m_RPM = 0;
    
    //Timer shooterHallEffectTimer = new Timer();
    
    double shooterHallEffectTimerNew = 0;
    double shooterHallEffectTimerOld = 0;
    
    double m_shooterHallEffectOldCount = 0;
    double m_shooterHallEffectNewCount = 0;
    double m_shooterHallEffectDeltaCount = 0;
    
    double m_shooterHallEffectOldCountPeriodic = 0;
    double m_shooterHallEffectNewCountPeriodic = 0;
    double m_shooterHallEffectDeltaCountPeriodic = 0;
    
    boolean m_shooterHallEffectOldCountGet = true;
    
    double shooterHallEffectTimerTime = 0;
    
    int shooterHallEffectRPM = 0;
    int m_shooterHallEffectRPMPeriodic = 0;
    
    boolean resetShooterHallEffectRPM = false;
     ...
     */
    public HallEffect(DigitalSource source) {
        if (source == null)
     ...
     */
    public HallEffect(int slot, int channel) {
        initCounter(Counter.Mode.kTwoPulse);
     ...
     */
    public HallEffect(CounterBase.EncodingType encodingType, DigitalSource upSource, DigitalSource downSource, boolean inverted) {
     ...
     */
    public HallEffect(AnalogTrigger trigger) {
        initCounter(Counter.Mode.kTwoPulse);
    ...
    public void setUpSource(DigitalSource source) {
        ...
        if (m_counter.readConfig_Mode() == Counter.Mode.kTwoPulse.value ||
                m_counter.readConfig_Mode() == Counter.Mode.kExternalDirection.value) {
            setUpSourceEdge(true, false);
        }
    ...
    public void setDownSource(DigitalSource source) {
        ...
        if(mode != Counter.Mode.kTwoPulse_val && mode != Counter.Mode.kExternalDirection_val) {
    ...
    public void setUpDownCounterMode() {
        m_counter.writeConfig_Mode(Counter.Mode.kTwoPulse.value);
    }
    ...
    public void setExternalDirectionMode() {
        m_counter.writeConfig_Mode(Counter.Mode.kExternalDirection.value);
    }
    ...
    public void setSemiPeriodMode(boolean highSemiPeriod) {
        m_counter.writeConfig_Mode(Counter.Mode.kSemiperiod.value);
    ...
    public void setPulseLengthMode(double threshold) {
        m_counter.writeConfig_Mode(Counter.Mode.kPulseLength.value);
    ...
    public void start() {
        ...
        m_Start_Time = Utility.getFPGATime() / 100000000;
        m_Prev_Time = m_Start_Time;
        m_Curr_Time = m_Start_Time;
        m_Prev_Count = get();
        m_Curr_Count = m_Prev_Count;
    }
    ...
    public void setReverseDirection(boolean reverseDirection) {
        if (m_counter.readConfig_Mode() == Counter.Mode.kExternalDirection.value) {
            if (reverseDirection) {
    ...
    public int returnRPM() {
        if (m_Return_RPM_Loop_Count >= 10) {
        m_Curr_Time = Utility.getFPGATime();
        m_Curr_Count = get();
        m_Diff_Time = m_Curr_Time - m_Prev_Time;
        m_Diff_Count = m_Curr_Count - m_Prev_Count;
        
        m_RPM = m_Diff_Count / m_Diff_Time * 60 * 1000000 / 2; //60 into per min, 4 to scale counts
        
        m_Prev_Time = m_Curr_Time;
        m_Prev_Count = m_Curr_Count;
        
        m_Return_RPM_Loop_Count = 0;
        
        
        
        }
        else {
            m_Return_RPM_Loop_Count++;
        }
    ...
    /**
     * Gets the Shooter RPM with the time based on the 20 millisecond time between the loop of the periodic thread
     * 
     * @return returns shooter RPM
     */
    public int getShooterHallEffectRPMPeriodic() {
   
            m_shooterHallEffectOldCountPeriodic = m_shooterHallEffectNewCountPeriodic;
            m_shooterHallEffectNewCountPeriodic = get();
            m_shooterHallEffectDeltaCountPeriodic = m_shooterHallEffectNewCountPeriodic - m_shooterHallEffectOldCountPeriodic;
            m_shooterHallEffectRPMPeriodic =(int) (60 * m_shooterHallEffectDeltaCountPeriodic / 0.02 / 2); // 0.02 = 20ms periodic loop time
            return m_shooterHallEffectRPMPeriodic;
    }
    
    public double getShooterHallEffectRadPerSec()
    {
    	return (returnRPM())*(2*3.1416/60);
    }
}
